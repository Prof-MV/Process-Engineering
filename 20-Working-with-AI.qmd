...
ggplot(rules, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = Rule), fontface = "bold", vjust = -1.5, size = 7 * diagram_text_scale, color = "white") +
  geom_text(aes(label = Description), vjust = 1, size = 5 * diagram_text_scale, color = "white", lineheight = 0.9) +
  ...
  theme(
    ...
    plot.subtitle = element_text(hjust = 0.5, size = 14 * diagram_text_scale)
  )
```
...
kable(motor_prompts, col.names = c("Quality Level", "Prompt", "Expected AI Response"),
  ...
  column_spec(3, width = if (knitr::is_html_output()) "45%" else NULL)
```
...
kable(plc_prompts, col.names = c("Quality Level", "Prompt", "Why This Works (or Doesn't)"),
  ...
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "10%" else NULL,
              color = c("red", "orange", "green")) %>%
  column_spec(2, width = if (knitr::is_html_output()) "50%" else NULL) %>%
...
ggplot(anatomy, aes(y = y_pos)) +
  ...
  geom_text(aes(x = 1, label = Component), fontface = "bold", size = 7 * diagram_text_scale, color = "white") +
  geom_label(aes(x = 2.2, label = stringr::str_wrap(Example, 50)),
             hjust = 0, size = 5 * diagram_text_scale, fill = "#f8f9fa", linewidth = 0) +
  geom_text(aes(x = 4.2, label = stringr::str_wrap(Purpose, 30)),
            hjust = 0, size = 5 * diagram_text_scale, color = "#2c3e50") +
  scale_fill_manual(values = c("#e74c3c", "#3498db", "#27ae60", "#f39c12",
                               "#9b59b6", "#1abc9c", "#34495e"), guide = "none") +
...
kable(rate_these, col.names = c("#", "Prompt", "Your Rating (B/M/G)", "Justification"),
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(2, width = if (knitr::is_html_output()) "50%" else NULL) %>%
  column_spec(3, width = if (knitr::is_html_output()) "15%" else NULL) %>%
  column_spec(4, width = if (knitr::is_html_output()) "25%" else NULL)
```
...
ggplot(steps_data, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = Step), fontface = "bold", vjust = -1.8, size = 5 * diagram_text_scale, color = "white") +
  geom_text(aes(label = Description), vjust = 0.8, size = 4 * diagram_text_scale, color = "white", lineheight = 0.9) +
  geom_text(aes(label = paste0("→ ", Key_Action)), vjust = 3.5, size = 3.5 * diagram_text_scale, color = "yellow", fontface = "italic") +
...
ggplot(tools_data, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = Tool), fontface = "bold", vjust = -2.2, size = 4 * diagram_text_scale, color = "white") +
  geom_text(aes(label = Best_For), vjust = 0.3, size = 3.5 * diagram_text_scale, color = "white", lineheight = 0.85) +
  geom_text(aes(label = Strengths), vjust = 3, size = 3 * diagram_text_scale, color = "lightyellow", lineheight = 0.85, fontface = "italic") +
...
kable(chat_uses, col.names = c("Use Case", "Description", "Example Prompt"),
  ...
  column_spec(2, width = if (knitr::is_html_output()) "35%" else NULL) %>%
  column_spec(3, width = if (knitr::is_html_output()) "47%" else NULL, monospace = TRUE)
```
...
kable(decision_data, col.names = c("Situation", "Best Tool", "Why"),
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, width = if (knitr::is_html_output()) "35%" else NULL) %>%
  column_spec(2, bold = TRUE, width = if (knitr::is_html_output()) "20%" else NULL, color = "#2980b9")
```
...
ggplot(cycle_data, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = Step), fontface = "bold", vjust = -1, size = 3.2 * diagram_text_scale, color = "white") +
  geom_text(aes(label = Description), vjust = 1.5, size = 2.8 * diagram_text_scale, color = "white", lineheight = 0.9) +
  # Add arrows between steps
...
kable(red_flags, col.names = c("Red Flag", "Example", "Your Response"),
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "20%" else NULL, color = "red")
```
...
kable(limitations, col.names = c("Limitation", "Description", "Example"),
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "18%" else NULL, color = "#e74c3c")
```
...
ggplot(summary_data, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = Principle), fontface = "bold", vjust = -1.2, size = 4 * diagram_text_scale, color = "white") +
  geom_text(aes(label = Key_Point), vjust = 1.2, size = 3 * diagram_text_scale, color = "white", lineheight = 0.9) +
