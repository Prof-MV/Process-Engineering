...
lean_def %>%
  kable(caption = "Lean Manufacturing Targets (Womack, Jones, Roos 1990)") %>%
...
mfg_compare %>%
  kable(caption = "Evolution of Manufacturing Systems") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered", "responsive"),
                full_width = knitr::is_html_output(), position = "center", font_size = 12) %>%
...
ggplot(timeline_data, aes(x = year, y = y)) +
  ...
  geom_text(aes(label = year), vjust = -2.5, fontface = "bold", size = 4 * diagram_text_scale) +
  geom_text(aes(label = event), vjust = 2.5, size = 3 * diagram_text_scale, lineheight = 0.8) +
  theme_void() +
...
ggplot() +
  ...
  annotate("text", x = 4, y = 5, label = "CUSTOMER FOCUS\nHighest Quality, Lowest Cost, Shortest Lead Time",
           fontface = "bold", size = 3.5 * diagram_text_scale, color = "white", lineheight = 0.8) +
  # Pillars
  annotate("rect", xmin = 0.5, xmax = 3.5, ymin = 1.5, ymax = 4.5, fill = "#3498DB", color = "black") +
  annotate("text", x = 2, y = 3, label = "JUST-IN-TIME\n\nRight part\nRight amount\nRight time",
           fontface = "bold", size = 3.5 * diagram_text_scale, color = "white", lineheight = 0.9) +
...
ggplot(ms_data, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = M), fontface = "bold", size = 6 * diagram_text_scale, color = "white", vjust = -0.3) +
  geom_text(aes(label = English), size = 4 * diagram_text_scale, color = "white", vjust = 1.2) +
...
three_ms %>%
  kable(caption = "The 3 M's Explained") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
...
ggplot(wastes, aes(x = reorder(waste, -impact), y = impact)) +
  ...
  geom_text(aes(label = letter), y = 0.3, size = 8 * diagram_text_scale, fontface = "bold", color = "white") +
  geom_text(aes(label = description, y = impact + 0.3), size = 3 * diagram_text_scale, vjust = 0, lineheight = 0.8) +
  ...
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 14 * diagram_text_scale),
    plot.subtitle = element_text(hjust = 0.5, size = 10 * diagram_text_scale),
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10 * diagram_text_scale, face = "bold"),
    panel.grid.minor = element_blank()
  )
```
...
wastes_detail %>%
  kable(caption = "The Seven Wastes: Detailed Analysis") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered", "responsive"),
...
ggplot(five_s) +
  ...
  annotate("text", x = 0, y = 0, label = "5S", size = 10 * diagram_text_scale, color = "white", fontface = "bold") +
  # Outer circles
  geom_point(aes(x = x, y = y, color = japanese), size = 22, alpha = 0.9) +
  geom_text(aes(x = x, y = y + 0.15, label = paste0(step, "S")),
            size = 5 * diagram_text_scale, fontface = "bold", color = "white") +
  geom_text(aes(x = x, y = y - 0.15, label = japanese),
            size = 3.5 * diagram_text_scale, color = "white") +
  # Labels outside
  geom_text(aes(x = x * 1.6, y = y * 1.6, label = english),
            size = 4 * diagram_text_scale, fontface = "bold") +
  geom_text(aes(x = x * 1.9, y = y * 1.9, label = action),
            size = 3 * diagram_text_scale, lineheight = 0.8) +
...
five_s_detail %>%
  kable(caption = "The 5S Steps Explained") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
ggplot(jit_elements, aes(x = x, y = y)) +
  ...
  geom_segment(aes(x = x + 0.3, xend = x + 0.7, y = y, yend = y),
               ...
               arrow = arrow(length = unit(0.2, "cm")), color = "#2C3E50", linewidth = 1.5) +
  # Circles
  geom_point(size = 20, color = "#3498DB") +
  geom_text(aes(label = element), size = 3 * diagram_text_scale, fontface = "bold", color = "white",
            lineheight = 0.8) +
  # Final output
  annotate("point", x = 6.5, y = 1, size = 25, color = "#2ECC71") +
  annotate("text", x = 6.5, y = 1, label = "CUSTOMER\nSATISFACTION",
           size = 3.5 * diagram_text_scale, fontface = "bold", color = "white", lineheight = 0.9) +
  annotate("segment", x = 5.4, xend = 5.9, y = 1, yend = 1,
           arrow = arrow(length = unit(0.3, "cm")), color = "#2C3E50", linewidth = 2) +
  theme_void() +
...
ggplot(combined, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = station), size = 3 * diagram_text_scale, fontface = "bold", color = "white") +
  # Inventory indicators
  geom_point(aes(y = y + 0.25, size = inventory), color = "#E74C3C", alpha = 0.7) +
  geom_text(aes(y = y + 0.25, label = inventory), size = 2.5 * diagram_text_scale, color = "white", fontface = "bold") +
  # Arrows
  geom_segment(data = combined[combined$x < 5,],
               aes(x = x + 0.45, xend = x + 0.55, y = y, yend = y),
               arrow = arrow(length = unit(0.15, "cm")), color = "#2C3E50") +
  # Labels
  annotate("text", x = 0.3, y = 2, label = "PUSH", hjust = 1, fontface = "bold", size = 5 * diagram_text_scale, color = "#E74C3C") +
  annotate("text", x = 0.3, y = 1, label = "PULL", hjust = 1, fontface = "bold", size = 5 * diagram_text_scale, color = "#2ECC71") +
  # Information flow arrows
  annotate("segment", x = 1, xend = 5, y = 2.5, yend = 2.5,
           arrow = arrow(length = unit(0.2, "cm")), color = "#F39C12", linewidth = 1) +
  annotate("text", x = 3, y = 2.6, label = "Information Flow →", color = "#F39C12", size = 3 * diagram_text_scale) +
  annotate("segment", x = 5, xend = 1, y = 0.5, yend = 0.5,
           arrow = arrow(length = unit(0.2, "cm")), color = "#2ECC71", linewidth = 1) +
  annotate("text", x = 3, y = 0.4, label = "← Signal/Kanban", color = "#2ECC71", size = 3 * diagram_text_scale) +
...
push_pull %>%
  kable(caption = "Push vs. Pull System Comparison") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
ggplot() +
  ...
  annotate("rect", xmin = 0, xmax = 7, ymin = 0, ymax = 4,
           fill = "white", color = "black", linewidth = 2) +
  # Header
  ...
  # Content
  annotate("text", x = 0.3, y = 2.8, label = "Part Number:", hjust = 0, size = 3.5 * diagram_text_scale) +
  annotate("text", x = 3, y = 2.8, label = "3278784", hjust = 0, size = 3.5 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 0.3, y = 2.4, label = "Part Name:", hjust = 0, size = 3.5 * diagram_text_scale) +
  annotate("text", x = 3, y = 2.4, label = "Securing Bracket", hjust = 0, size = 3.5 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 0.3, y = 2.0, label = "From Process:", hjust = 0, size = 3.5 * diagram_text_scale) +
  annotate("text", x = 3, y = 2.0, label = "CNC Turning (CNC-6)", hjust = 0, size = 3.5 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 0.3, y = 1.6, label = "To Process:", hjust = 0, size = 3.5 * diagram_text_scale) +
  annotate("text", x = 3, y = 1.6, label = "CNC Machining (CNC-M7)", hjust = 0, size = 3.5 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 0.3, y = 1.0, label = "Container Qty:", hjust = 0, size = 3.5 * diagram_text_scale) +
  annotate("text", x = 2.5, y = 1.0, label = "12", hjust = 0, size = 4 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 4, y = 1.0, label = "Container Type:", hjust = 0, size = 3.5 * diagram_text_scale) +
  annotate("text", x = 6, y = 1.0, label = "A1", hjust = 0, size = 4 * diagram_text_scale, fontface = "bold") +
  # Barcode placeholder
  annotate("rect", xmin = 5, xmax = 6.8, ymin = 1.8, ymax = 2.8, fill = "#ECF0F1", color = "black") +
  annotate("text", x = 5.9, y = 2.3, label = "|||||||||||", size = 5 * diagram_text_scale) +
  theme_void() +
  labs(title = "Sample Production Kanban Card") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12 * diagram_text_scale))
```
...
ggplot() +
  ...
  annotate("text", x = 2.25, y = 0.75, label = "Op 1", color = "white", size = 3 * diagram_text_scale) +
  annotate("text", x = 4.25, y = 0.75, label = "Op 2", color = "white", size = 3 * diagram_text_scale) +
  annotate("text", x = 6.25, y = 0.75, label = "Op 3", color = "white", size = 3 * diagram_text_scale) +
  annotate("text", x = 8, y = 0.75, label = "Op 4", color = "white", size = 3 * diagram_text_scale) +
  ...
  annotate("segment", x = 0, xend = 10, y = 3.5, yend = 3.5,
           linetype = "dashed", color = "#2ECC71", linewidth = 1.5) +
...
ggplot() +
  ...
  geom_text(data = before, aes(x = (x_start + x_end)/2, y = 1.8, label = activity),
            size = 2.5 * diagram_text_scale, angle = 0) +
  # After SMED bars
...
smed_steps %>%
  kable(caption = "The Four Steps of SMED") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
  row_spec(0, bold = TRUE, background = "#8E44AD", color = "white") %>%
...
ggplot(pdca) +
  ...
  annotate("text", x = 0, y = 0, label = "KAIZEN\n改善", size = 6 * diagram_text_scale, color = "white",
           fontface = "bold", lineheight = 0.8) +
  # Outer circles
  geom_point(aes(x = x, y = y, color = step), size = 25, alpha = 0.9) +
  geom_text(aes(x = x, y = y + 0.1, label = step), size = 5 * diagram_text_scale, fontface = "bold", color = "white") +
  geom_text(aes(x = x, y = y - 0.25, label = description), size = 3 * diagram_text_scale, color = "white", lineheight = 0.8) +
  # Arrows
  annotate("curve", x = 1.5, xend = 1.5, y = 2, yend = -0.5,
           arrow = arrow(length = unit(0.3, "cm")), curvature = 0.5, color = "#2C3E50", linewidth = 1) +
...
vsm_metrics %>%
  kable(caption = "Key Metrics in Value Stream Maps") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
summary_data %>%
  kable(caption = "Lean Manufacturing: Key Concepts Summary") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
  row_spec(0, bold = TRUE, background = "#2C3E50", color = "white") %>%
