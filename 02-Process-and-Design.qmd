...
automation_data <- data.frame(
  ...
  # Direct scale like every other metric here (5 = high), matching the
  # Operating Cost row of the automation-selection table below: Fixed is
  # lowest, Programmable is medium, Flexible is low-medium (between them).
  Unit_Cost = c(1, 3, 2)
)
...
cat("==========================\n")
cat(sprintf("Equipment cost: $%s\n", format(fixed_automation_cost, big.mark = " ", scientific = FALSE)))
cat(sprintf("Annual production: %s parts\n", format(annual_production, big.mark = " ", scientific = FALSE)))
...
automation_guide %>%
  kable(caption = "Automation Type Selection Guide") %>%
  kable_styling(
...
pros_cons %>%
  kable(caption = "Advantages and Disadvantages of Manufacturing Automation") %>%
...
ggplot() +
  ...
  geom_tile(data = phases, aes(x = x, y = 1, fill = color),
            width = 0.9, height = 0.5, color = "white", linewidth = 1) +
  ...
  geom_text(data = gates, aes(x = x, y = 0.6, label = Gate),
            fontface = "bold", size = 4 * diagram_text_scale, color = "#C0392B") +
  geom_text(data = gates, aes(x = x, y = 0.3, label = label),
            size = 2.5 * diagram_text_scale, color = "#7F8C8D") +
  ...
  annotate("text", x = 4.5, y = 1.6,
           ...
           fontface = "italic", size = 4 * diagram_text_scale) +
  scale_fill_identity() +
...
disciplines %>%
  kable(caption = "Engineering Disciplines and Deliverables") %>%
...
activities %>%
  kable(caption = "Later Project Phases") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"), full_width = knitr::is_html_output()) %>%
  row_spec(0, bold = TRUE, background = "#2C3E50", color = "white") %>%
  column_spec(1, bold = TRUE, background = "#ECF0F1")
```
...
pfd_elements %>%
  kable(caption = "Process Flow Diagram (PFD) Content") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
...
ggplot(symbols, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = Name), y = symbols$y + 0.15, fontface = "bold", size = 3 * diagram_text_scale) +
  geom_text(aes(label = Symbol_Code), y = symbols$y - 0.1, size = 3.5 * diagram_text_scale,
...
instrument_tags %>%
  kable(caption = "ISA Instrument Tag Structure") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"), full_width = knitr::is_html_output()) %>%
...
dfm_guide %>%
  kable(caption = "Design for Manufacturing Guidelines") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"), full_width = knitr::is_html_output()) %>%
  row_spec(0, bold = TRUE, background = "#1A5276", color = "white") %>%
...
```{r hazop-study, echo=FALSE, results='asis'}
  embed_youtube("6AqtX8oCpKI", "What is a HAZOP? A Crash Course")
```

```{r hazop-study-overview, echo=FALSE, results='asis'}
  embed_youtube("ukn0RRt0ReA", "Key P&ID Details for HAZOPs")
```
...
hazop_keywords %>%
  kable(caption = "HAZOP Guide Words and Examples") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"), full_width = knitr::is_html_output()) %>%
...
hazop_example %>%
  kable(caption = "HAZOP Worksheet Example") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), font_size = 11) %>%
  row_spec(0, bold = TRUE, background = "#C0392B", color = "white") %>%
...
cost_classes %>%
  kable(caption = "Cost Estimate Classification (AACE International)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"), full_width = knitr::is_html_output()) %>%
  row_spec(0, bold = TRUE, background = "#2C3E50", color = "white") %>%
  column_spec(1, bold = TRUE, background = "#ECF0F1")
```
...
ggplot(timeline) +
  geom_segment(aes(x = Start, xend = Start + Duration, y = y, yend = y),
               linewidth = 10, color = "#3498DB", lineend = "round") +
...
summary_ch2 %>%
  kable(caption = "Chapter 2 Summary") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"), full_width = knitr::is_html_output()) %>%
  row_spec(0, bold = TRUE, background = "#1A5276", color = "white") %>%
