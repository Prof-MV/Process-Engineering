...
ggplot() +
  ...
  geom_label(data = components,
             ...
             size = 4.5 * diagram_text_scale, fontface = "bold", color = "white",
...
ggplot(scenarios, aes(x = x, y = y)) +
  ...
  theme(
    plot.title = element_text(face = "bold", size = 14 * diagram_text_scale),
    strip.text = element_text(face = "bold", size = 10 * diagram_text_scale),
...
ggplot(linearity_data, aes(x = Reference, y = Bias)) +
  geom_point(color = "steelblue", size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", formula = y ~ x, color = "red", se = TRUE, alpha = 0.2) +
  ...
  annotate("text", x = 60, y = 0.04,
           ...
           hjust = 0, size = 4 * diagram_text_scale, fontface = "bold") +
...
kable(rr_defs, col.names = c("Component", "Also Known As", "Definition", "Caused By"),
      caption = if (knitr::is_latex_output()) "Gauge R\\&R Components" else "Gauge R&R Components") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
...
kable(grr_design, caption = if (knitr::is_latex_output()) "Standard Gauge R\\&R Study Design Parameters" else "Standard Gauge R&R Study Design Parameters") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "20%" else NULL)
```
...
kable(acceptance, caption = if (knitr::is_latex_output()) "AIAG Gauge R\\&R Acceptance Guidelines" else "AIAG Gauge R&R Acceptance Guidelines") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
...
sd_good <- 0.3
good_measurement <- true_process + rnorm(n, 0, sd_good)

# Poor measurement system (high measurement error)
sd_poor <- 5
poor_measurement <- true_process + rnorm(n, 0, sd_poor)

# %GRR as a share of total observed variation (process sigma = 2)
grr_good <- round(100 * sd_good / sqrt(2^2 + sd_good^2))
grr_poor <- round(100 * sd_poor / sqrt(2^2 + sd_poor^2))
lab_good <- paste0("Good MSA (%GRR = ", grr_good, "%)")
lab_poor <- paste0("Poor MSA (%GRR = ", grr_poor, "%)")
...
chart_data <- data.frame(
  ...
  System = factor(rep(c(lab_good, lab_poor), each = n), levels = c(lab_good, lab_poor))
)

limits_data <- data.frame(
  System = factor(c(lab_good, lab_poor), levels = c(lab_good, lab_poor)),
...
ggplot(chart_data, aes(x = Sample, y = Value)) +
  ...
  labs(title = "Impact of Measurement System Quality on Control Charts",
       subtitle = "Same stable process measured with different gauge quality (same y-axis scale on both charts)",
...
ggplot(steps, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = name), fontface = "bold", size = 3.5 * diagram_text_scale, color = "white",
            vjust = -0.3) +
  geom_text(aes(label = description), size = 2.8 * diagram_text_scale, color = "white",
            vjust = 1.2) +
...
kable(head(template, 12),
      caption = if (knitr::is_latex_output()) "Gauge R\\&R Data Collection Sheet (Partial Example)" else "Gauge R&R Data Collection Sheet (Partial Example)") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) %>%
  column_spec(1:4, width = if (knitr::is_html_output()) "20%" else NULL)
```
