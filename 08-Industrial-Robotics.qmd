...
ggplot(benefits, aes(x = reorder(Benefit, Impact), y = Impact, fill = Benefit)) +
  ...
  geom_text(aes(label = Description), hjust = -0.05, size = 3 * diagram_text_scale) +
  coord_flip() +
...
ggplot(market_data, aes(x = Year, y = Installations)) +
  ...
  geom_text(aes(label = paste0(Installations, "K")), vjust = -0.5, size = 4 * diagram_text_scale) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "#e74c3c", linetype = "dashed") +
...
ggplot(robot_types, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = Type), fontface = "bold", vjust = -2, size = 4 * diagram_text_scale, color = "white") +
  geom_text(aes(label = Characteristics), vjust = 0, size = 2.8 * diagram_text_scale, color = "white", lineheight = 0.85) +
  geom_text(aes(label = Applications), vjust = 3.5, size = 2.5 * diagram_text_scale, color = "white",
...
ggplot() +
  ...
  annotate("segment", x = 1, xend = 1, y = 0, yend = 1.5, linewidth = 8, color = "#34495e") +
  annotate("segment", x = 1, xend = 2.5, y = 1.5, yend = 2.5, linewidth = 6, color = "#3498db") +
  annotate("segment", x = 2.5, xend = 4, y = 2.5, yend = 2, linewidth = 5, color = "#27ae60") +
  annotate("segment", x = 4, xend = 5, y = 2, yend = 2.2, linewidth = 4, color = "#e74c3c") +
  ...
  # Axis labels
  annotate("text", x = 0.5, y = 0, label = "J1\n(Base)", size = 3 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 0.5, y = 1.5, label = "J2\n(Shoulder)", size = 3 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 2.5, y = 3, label = "J3\n(Elbow)", size = 3 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 4, y = 1.4, label = "J4\n(Wrist 1)", size = 3 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 5.5, y = 2.2, label = "J5, J6\n(Wrist 2,3)", size = 3 * diagram_text_scale, fontface = "bold") +
  ...
  annotate("text", x = 6.2, y = 2.3, label = "J1: Base rotation", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 6.2, y = 2.0, label = "J2: Shoulder pivot", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 6.2, y = 1.7, label = "J3: Elbow pivot", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 6.2, y = 1.4, label = "J4: Wrist rotation", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 6.2, y = 1.1, label = "J5: Wrist bend", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 6.2, y = 0.8, label = "J6: Tool rotation", hjust = 0, size = 3 * diagram_text_scale) +
...
ggplot() +
  ...
  annotate("text", x = 900, y = 100, label = "Max Reach: 1800mm", size = 3.5 * diagram_text_scale) +
...
total_load <- gripper_weight + part_weight          # what the robot actually carries
required_capacity <- total_load * safety_factor      # total load, with margin applied
remaining_margin <- robot_max_payload - total_load   # headroom before the safety factor
...
cat("Total load (gripper + part):", total_load, "kg\n")
cat("Remaining margin:", remaining_margin, "kg (",
    round(100 * remaining_margin / robot_max_payload), "%)\n")
...
tolerance_circle <- data.frame(x0 = 0, y0 = 0, r = 0.05)

ggplot(scenarios, aes(x = x, y = y)) +
  geom_point(color = "steelblue", size = 2, alpha = 0.7) +
  annotate("point", x = 0, y = 0, color = "red", size = 4, shape = 3, stroke = 2) +
  geom_circle(data = tolerance_circle, aes(x0 = x0, y0 = y0, r = r),
...
ggplot() +
  ...
  annotate("segment", x = 3, xend = 7, y = 2.75, yend = 2.75,
           arrow = arrow(ends = "both", length = unit(0.2, "cm")), linewidth = 1, linetype = "dashed") +
...
kable(eoat, col.names = c("EOAT Type", "Operating Principle", "Applications", "Key Considerations"),
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "16%" else NULL)
```
...
ggplot() +
  ...
  annotate("text", x = 6, y = 3.8, label = "Tool Frame\n(Moves with robot)", size = 3 * diagram_text_scale,
           lineheight = 0.9, fontface = "bold") +
  ...
  annotate("text", x = 8.75, y = 0.5, label = "User/Work Object Frame\n(On fixture or part)",
           size = 3 * diagram_text_scale, lineheight = 0.9, fontface = "bold") +

  # Robot arm (simplified)
  annotate("segment", x = 1, xend = 3, y = 1, yend = 3, linewidth = 4, color = "#3498db") +
  annotate("segment", x = 3, xend = 5, y = 3, yend = 4, linewidth = 3, color = "#3498db") +
  annotate("segment", x = 5, xend = 6, y = 4, yend = 4.5, linewidth = 2, color = "#3498db") +
  ...
  annotate("text", x = 0.2, y = 7.2, label = "Coordinate Types:", hjust = 0,
           fontface = "bold", size = 4 * diagram_text_scale) +
  annotate("text", x = 0.2, y = 6.7, label = "• Joint: J1, J2, J3, J4, J5, J6 (degrees)", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 0.2, y = 6.3, label = "• Cartesian: X, Y, Z, Rx, Ry, Rz (mm, degrees)", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 0.2, y = 5.9, label = "• Tool: Relative to end effector", hjust = 0, size = 3 * diagram_text_scale) +
...
ggplot(auto_apps, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = Application), fontface = "bold", size = 4 * diagram_text_scale, color = "white", vjust = -1.5) +
  geom_text(aes(label = paste0("Robot: ", Robot_Type)), size = 2.5 * diagram_text_scale, color = "white", vjust = 0) +
  geom_text(aes(label = Key_Metrics), size = 2.5 * diagram_text_scale, color = "white",
...
kable(food_apps, col.names = c("Application", "Description", "Robot Type", "Special Requirements"),
      caption = if (knitr::is_latex_output()) "Food \\& Beverage Robot Applications" else "Food & Beverage Robot Applications") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "16%" else NULL)
```
...
ggplot() +
  ...
  annotate("rect", xmin = 2, xmax = 8, ymin = 1, ymax = 6,
           fill = "#f8f9fa", color = "black", linetype = "dashed", linewidth = 1) +

  # Robot
  annotate("point", x = 5, y = 3.5, size = 15, color = "#3498db") +
  annotate("text", x = 5, y = 3.5, label = "Robot", color = "white",
           fontface = "bold", size = 3 * diagram_text_scale) +

  # Physical barrier (fence)
  annotate("segment", x = 2, xend = 2, y = 1, yend = 6, color = "#e74c3c", linewidth = 3) +
  annotate("segment", x = 8, xend = 8, y = 1, yend = 3, color = "#e74c3c", linewidth = 3) +
  annotate("segment", x = 8, xend = 8, y = 4.5, yend = 6, color = "#e74c3c", linewidth = 3) +
  annotate("text", x = 1.5, y = 3.5, label = "Safety\nFence",
           color = "#e74c3c", fontface = "bold", size = 3 * diagram_text_scale, lineheight = 0.9) +

  # Light curtain
  annotate("segment", x = 8, xend = 8, y = 3, yend = 4.5,
           color = "#f39c12", linewidth = 2, linetype = "dotted") +
  ...
  annotate("text", x = 9, y = 3.75, label = "Light\nCurtain",
           color = "#f39c12", fontface = "bold", size = 3 * diagram_text_scale, lineheight = 0.9) +

  # Interlocked gate -- sits on the fence line (y = 1), label just inside
  annotate("rect", xmin = 4, xmax = 6, ymin = 0.8, ymax = 1.2,
           fill = "#27ae60", color = "black") +
  annotate("text", x = 5, y = 1.5, label = "Interlocked Gate",
           color = "#27ae60", fontface = "bold", size = 3 * diagram_text_scale) +

  # Safety mat -- outside the fence, on the approach to the gate: stepping on
  # it (before the gate can even be opened) triggers a stop, not a device
  # sitting inside the hazard zone it is meant to keep people out of.
  annotate("rect", xmin = 4, xmax = 6, ymin = 0.35, ymax = 0.75,
           fill = "#9b59b6", alpha = 0.5, color = "#9b59b6") +
  annotate("text", x = 5, y = 0.15, label = "Safety Mat",
           color = "#9b59b6", fontface = "bold", size = 2.5 * diagram_text_scale) +

  # E-stop
  annotate("point", x = 1.5, y = 5.5, size = 8, color = "red") +
  annotate("text", x = 1.5, y = 5.5, label = "E", color = "white",
           fontface = "bold", size = 3 * diagram_text_scale) +
  annotate("text", x = 1.5, y = 5, label = "E-Stop", size = 2.5 * diagram_text_scale) +
  ...
  annotate("text", x = 7, y = 1.3, label = "Area\nScanner",
           color = "#1abc9c", fontface = "bold", size = 2.5 * diagram_text_scale, lineheight = 0.9) +

  # Legend
  annotate("rect", xmin = 9.5, xmax = 13, ymin = 1, ymax = 6,
           fill = "#f8f9fa", color = "gray") +
  annotate("text", x = 9.7, y = 5.7, label = "Safeguarding Devices:",
           hjust = 0, fontface = "bold", size = 3.5 * diagram_text_scale) +
  annotate("segment", x = 9.7, xend = 10.2, y = 5.3, yend = 5.3, color = "#e74c3c", linewidth = 2) +
  annotate("text", x = 10.4, y = 5.3, label = "Physical Barrier", hjust = 0, size = 2.8 * diagram_text_scale) +
  annotate("segment", x = 9.7, xend = 10.2, y = 4.9, yend = 4.9,
           color = "#f39c12", linewidth = 2, linetype = "dotted") +
  annotate("text", x = 10.4, y = 4.9, label = "Light Curtain", hjust = 0, size = 2.8 * diagram_text_scale) +
  annotate("rect", xmin = 9.7, xmax = 10.2, ymin = 4.4, ymax = 4.6, fill = "#27ae60") +
  annotate("text", x = 10.4, y = 4.5, label = "Interlocked Gate", hjust = 0, size = 2.8 * diagram_text_scale) +
  annotate("rect", xmin = 9.7, xmax = 10.2, ymin = 4, ymax = 4.2, fill = "#9b59b6", alpha = 0.5) +
  annotate("text", x = 10.4, y = 4.1, label = "Safety Mat", hjust = 0, size = 2.8 * diagram_text_scale) +
  annotate("point", x = 9.95, y = 3.7, size = 4, color = "#1abc9c") +
  annotate("text", x = 10.4, y = 3.7, label = "Area Scanner", hjust = 0, size = 2.8 * diagram_text_scale) +
  annotate("point", x = 9.95, y = 3.3, size = 4, color = "red") +
  annotate("text", x = 10.4, y = 3.3, label = "E-Stop", hjust = 0, size = 2.8 * diagram_text_scale) +
