...
ggplot(industry_data, aes(x = reorder(Industry, Cost_Per_Hour), y = Cost_Per_Hour)) +
  ...
  geom_text(aes(label = paste0("$", format(Cost_Per_Hour, big.mark = " ", scientific = FALSE, trim = TRUE))),
            hjust = -0.1, size = 4 * diagram_text_scale) +
  coord_flip() +
...
kable(approach_comparison, col.names = c("Aspect", "Reactive Approach", "Proactive Approach"),
  ...
  column_spec(2, background = "#ffcccc", width = if (knitr::is_html_output()) "42%" else NULL) %>%
  column_spec(3, background = "#ccffcc", width = if (knitr::is_html_output()) "43%" else NULL)
```
...
ggplot(steps, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = step), color = "white", size = 8 * diagram_text_scale, fontface = "bold") +
  geom_text(aes(label = name, y = y + 0.25), size = 4 * diagram_text_scale, fontface = "bold") +
  geom_text(aes(label = description, y = y - 0.25), size = 3 * diagram_text_scale) +
  scale_x_continuous(limits = c(0.5, 6.8)) +
  scale_y_continuous(limits = c(0.5, 1.5)) +
  labs(title = "Systematic Troubleshooting Process") +
  theme_void() +
  theme(plot.title = element_text(size = 16 * diagram_text_scale, face = "bold", hjust = 0.5))
```
...
kable(w2h, col.names = c("Question", "What to Ask", "Example Response"),
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, color = "white", background = "#2c3e50", width = if (knitr::is_html_output()) "12%" else NULL) %>%
  column_spec(2, width = if (knitr::is_html_output()) "40%" else NULL) %>%
  column_spec(3, width = if (knitr::is_html_output()) "48%" else NULL, background = "#f8f9fa")
```
...
ggplot(timeline_data, aes(x = date, y = 0)) +
  ...
  geom_text(aes(label = format(date, "%b %d")), vjust = 2, size = 2.5 * diagram_text_scale) +
...
ggplot(levels, aes(x = 0, y = y)) +
  ...
  geom_text(aes(label = level), fontface = "bold", size = 5 * diagram_text_scale, color = "white") +
  ...
  annotate("text", x = 4, y = 4, label = "Example: Motor is overheating",
           hjust = 0, size = 3.5 * diagram_text_scale, fontface = "italic") +
  ...
  theme_void() +
  theme(plot.title = element_text(face = "bold", size = 14 * diagram_text_scale, hjust = 0.5),
...
ggplot(whys, aes(x = 1, y = y)) +
  ...
  geom_text(aes(x = -0.8, label = question), hjust = 1, fontface = "bold", size = 4 * diagram_text_scale) +
  geom_text(aes(label = answer), hjust = 0.5, size = 3.5 * diagram_text_scale,
...
ggplot() +
  ...
  annotate("text", x = -3, y = 0.3, label = "Root Cause A:\nMissing parameter documentation",
           fontface = "bold", size = 3.5 * diagram_text_scale, color = "#3498db") +
  annotate("text", x = 2, y = 0.3, label = "Root Cause B:\nSchedule pressure affecting quality",
           fontface = "bold", size = 3.5 * diagram_text_scale, color = "#27ae60") +
...
ggplot(categories, aes(x = x, y = 1)) +
  ...
  geom_text(aes(label = category), y = 1.3, fontface = "bold", size = 4 * diagram_text_scale, color = "white") +
  geom_text(aes(label = examples), y = 0.7, size = 2.8 * diagram_text_scale, color = "white") +
...
ggplot() +
  # Main spine
  annotate("segment", x = spine$x[1], y = spine$y[1], xend = spine$x[2], yend = spine$y[2],
           linewidth = 2, color = "gray30") +
  # Problem box
  geom_label(data = problem, aes(x = x, y = y, label = label),
             fill = "#e74c3c", color = "white", fontface = "bold",
             size = 5 * diagram_text_scale, label.padding = unit(0.5, "lines")) +
  # Category bones
  ...
  geom_label(data = bones, aes(x = x_start, y = y_start, label = category),
             fill = "steelblue", color = "white", fontface = "bold", size = 4 * diagram_text_scale) +
  # Sub-cause labels
...
kable(tips, caption = "Best Practices for Fishbone Diagram Development") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "28%" else NULL)
```
...
kable(symbols, caption = "Fault Tree Analysis Symbols") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "12%" else NULL) %>%
  column_spec(2, width = if (knitr::is_html_output()) "18%" else NULL)
```
...
ggplot() +
  ...
  annotate("text", x = c(7, 3, 7, 11, 7), y = c(9, 6, 6, 6, 3),
           label = "OR", size = 2 * diagram_text_scale, fontface = "bold") +
  ...
  geom_text(data = fta_nodes[fta_nodes$type == "intermediate",],
            ...
            color = "white", size = 2.8 * diagram_text_scale, fontface = "bold") +
  ...
  geom_text(data = fta_nodes[fta_nodes$type == "basic",],
            aes(x = x, y = y, label = label),
            color = "white", size = 2.5 * diagram_text_scale) +
  # Top event styling
  ...
  geom_text(aes(x = 7, y = 10, label = "Pump fails to\ndeliver flow"),
            color = "white", size = 3.5 * diagram_text_scale, fontface = "bold") +
...
ggplot(defects, aes(x = Defect_Type)) +
  ...
  geom_point(aes(y = Cum_Percent * max(Count) / 100),
             color = "#e74c3c", size = 3) +
  geom_text(aes(y = Count, label = Count), vjust = -0.5, size = 3.5 * diagram_text_scale) +
  geom_text(aes(y = Cum_Percent * max(Count) / 100,
                label = paste0(round(Cum_Percent, 0), "%")),
            vjust = -1, hjust = -0.2, size = 3 * diagram_text_scale, color = "#e74c3c") +
  geom_hline(yintercept = 80 * max(defects$Count) / 100,
             linetype = "dashed", color = "gray50") +
  annotate("text", x = 7.5, y = 80 * max(defects$Count) / 100 + 5,
           label = "80% line", size = 3.5 * diagram_text_scale, color = "gray40") +
  scale_y_continuous(
    name = "Defect Count",
...
ggplot(scratch_causes, aes(x = Cause)) +
  ...
  geom_point(aes(y = Cum_Pct * max(Count) / 100),
             color = "#e74c3c", size = 3) +
  geom_text(aes(y = Count, label = Count), vjust = -0.5, size = 4 * diagram_text_scale) +
...
kable(is_isnot, col.names = c("Dimension", "Question", "IS", "IS NOT", "What's Different?"),
  ...
  column_spec(2, width = if (knitr::is_html_output()) "20%" else NULL) %>%
  column_spec(3, background = "#d4edda", width = if (knitr::is_html_output()) "22%" else NULL) %>%
  column_spec(4, background = "#f8d7da", width = if (knitr::is_html_output()) "22%" else NULL) %>%
  column_spec(5, background = "#fff3cd", width = if (knitr::is_html_output()) "28%" else NULL)
```
...
ggplot(d8_steps, aes(x = 1, y = y)) +
  ...
  geom_text(aes(x = -1.5, label = D), fontface = "bold", size = 6 * diagram_text_scale, color = "steelblue") +
  geom_text(aes(x = -0.5, label = Name), fontface = "bold", size = 4 * diagram_text_scale, hjust = 0) +
  geom_text(aes(x = 1.3, label = Description), size = 3.2 * diagram_text_scale, hjust = 0) +
...
ggplot(ca_types, aes(x = reorder(type, reliability), y = reliability)) +
  ...
  geom_text(aes(label = paste0(reliability, "%")), hjust = -0.2, size = 4 * diagram_text_scale) +
  geom_text(aes(y = 5, label = example), hjust = 0, size = 3 * diagram_text_scale, color = "white") +
  scale_fill_identity() +
  coord_flip() +
  ...
  theme(
    plot.title = element_text(face = "bold", size = 14 * diagram_text_scale),
    axis.text.y = element_text(size = 11 * diagram_text_scale)
  )
```
...
kable(smart, col.names = c("", "Meaning", "Description", "Poor Example", "Good Example"),
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, background = "#4682B4", color = "white", width = if (knitr::is_html_output()) "4%" else NULL) %>%
...
ggplot(effectiveness_data, aes(x = Date, y = Defect_Rate)) +
  ...
  annotate("text", x = ca_date, y = max(defect_rate) * 0.95,
           ...
           fontface = "bold", color = "red", size = 3.5 * diagram_text_scale) +
  ...
  annotate("text", x = min(dates), y = mean(defect_rate[1:14]) + 0.3,
           ...
           hjust = 0, size = 3 * diagram_text_scale, color = "orange") +
  ...
  annotate("text", x = max(dates), y = mean(defect_rate[15:30]) + 0.3,
           ...
           hjust = 1, size = 3 * diagram_text_scale, color = "darkgreen") +
...
kable(d8_report, col.names = c("D#", "Discipline", "Summary"),
  ...
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = knitr::is_html_output()) %>%
  column_spec(1, bold = TRUE, width = if (knitr::is_html_output()) "6%" else NULL) %>%
  column_spec(2, bold = TRUE, width = if (knitr::is_html_output()) "15%" else NULL)
```
