...
ggplot(process_categories, aes(x = x_pos, y = 1)) +
  ...
  geom_text(aes(label = Category), color = "white", fontface = "bold", size = 4 * diagram_text_scale) +
...
ggplot(selection_data, aes(x = Volume, y = Complexity, color = Category)) +
  ...
  ggrepel::geom_text_repel(aes(label = Process),
                           size = 3 * diagram_text_scale, fontface = "bold",
                           seed = 42, box.padding = 0.6, point.padding = 0.4,
...
comparison_data %>%
  kable(caption = "Manufacturing Process Comparison Summary") %>%
  kable_styling(
...
for(i in 1:nrow(cost_analysis)) {
  cat(sprintf("At %s parts: $%.2f per part (Tooling: $%.2f, Material: $%.2f, Processing: $%.2f)\n",
              format(cost_analysis$Volume[i], big.mark = " ", scientific = FALSE),
...
ggplot(force_data, aes(x = x, y = 1)) +
  ...
  geom_text(aes(label = Type), y = 1.1, fontface = "bold", size = 4 * diagram_text_scale) +
  geom_text(aes(label = Process), y = 0.9, size = 3 * diagram_text_scale, color = "gray30") +
...
joining_data %>%
  kable(caption = "Joining Process Comparison") %>%
  kable_styling(
...
coating_data %>%
  kable(caption = "Surface Treatment and Coating Processes") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "bordered"),
    full_width = knitr::is_html_output()
  ) %>%
...
industry_examples %>%
  kable(caption = "Manufacturing Process Applications by Industry") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "bordered"),
    full_width = knitr::is_html_output()
  ) %>%
...
summary_data %>%
  kable(caption = "Manufacturing Processes Summary") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "bordered"),
    full_width = knitr::is_html_output()
  ) %>%
