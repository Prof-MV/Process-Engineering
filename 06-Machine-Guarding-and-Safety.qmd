...
ggplot(hierarchy_data, aes(x = 0, y = y)) +
  ...
  geom_text(aes(x = 3.5, label = description), hjust = 0, size = 3.5 * diagram_text_scale, color = "#2C3E50") +
  # Effectiveness arrow
  ...
  annotate("text", x = -4.5, y = 3, label = "More\nEffective",
           angle = 90, vjust = 0, fontface = "bold", size = 4 * diagram_text_scale) +
...
hierarchy_detail %>%
  kable(caption = "Hierarchy of Controls: Detailed Breakdown") %>%
...
ggplot(guard_long, aes(x = type, y = value, fill = metric)) +
  ...
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 14 * diagram_text_scale),
    plot.subtitle = element_text(hjust = 0.5, size = 10 * diagram_text_scale),
    axis.text.x = element_text(angle = 20, hjust = 1, size = 10 * diagram_text_scale),
...
fixed_guards %>%
  kable(caption = "Fixed Guards: Key Characteristics") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
ggplot() +
  ...
  annotate("rect", xmin = 2.8, xmax = 3, ymin = 1.5, ymax = 3.5,
           fill = "#F39C12", color = "black", linewidth = 1) +
  annotate("text", x = 1.8, y = 2.5, label = "Guard\nDoor", size = 3 * diagram_text_scale) +
  # Interlock switch
  ...
  annotate("text", x = 1, y = 1, label = "Safety\nPLC", size = 3 * diagram_text_scale, fontface = "bold") +
  # Connection lines
  ...
  annotate("segment", x = 1.5, xend = 3, y = 1, yend = 1,
           arrow = arrow(length = unit(0.2, "cm")), color = "#2ECC71", linewidth = 1) +
  # Labels
  annotate("text", x = 2.25, y = 0.7, label = "Stop Signal", size = 3 * diagram_text_scale, color = "#2ECC71") +
  # Status indicators
  annotate("rect", xmin = 8, xmax = 10, ymin = 3, ymax = 4, fill = "#D5F5E3", color = "black") +
  annotate("text", x = 9, y = 3.5, label = "Door Closed\n= Machine Runs", size = 3 * diagram_text_scale) +
  annotate("rect", xmin = 8, xmax = 10, ymin = 1.5, ymax = 2.5, fill = "#FADBD8", color = "black") +
  annotate("text", x = 9, y = 2, label = "Door Open\n= Machine Stops", size = 3 * diagram_text_scale) +
  theme_void() +
...
stop_cat %>%
  kable(caption = "Machine Stop Categories (IEC 60204-1)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
ggplot(loto_steps, aes(x = x, y = 1)) +
  ...
  geom_text(aes(label = step), color = "white", fontface = "bold", size = 6 * diagram_text_scale) +
  # Action labels
  geom_text(aes(y = 0.5, label = action), size = 3 * diagram_text_scale, lineheight = 0.8) +
  # Arrows
  geom_segment(data = loto_steps[1:4,],
               aes(x = x + 0.4, xend = x + 0.6, y = 1, yend = 1),
               arrow = arrow(length = unit(0.2, "cm")), color = "#2C3E50", linewidth = 1) +
  # Return arrow
  ...
  annotate("text", x = 3, y = 1.85, label = "To restart: Reverse sequence",
           color = "#27AE60", fontface = "italic", size = 3 * diagram_text_scale) +
  theme_void() +
...
ggplot() +
  ...
  annotate("text", x = -1, y = 0.5, label = "Resolution\n(14mm typical)", size = 2.5 * diagram_text_scale) +
  theme_void() +
...
lc_specs %>%
  kable(caption = "Light Curtain Specifications by Application") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
ggplot() +
  ...
  annotate("text", x = 0, y = -0.5, label = "Scanner", fontface = "bold", size = 3 * diagram_text_scale) +
  # Zone labels
  annotate("text", x = 2.5, y = 2.5, label = "WARNING ZONE\n(Machine slows/alarms)",
           color = "#F39C12", fontface = "bold", size = 3.5 * diagram_text_scale) +
  annotate("text", x = 1.2, y = 1.2, label = "SAFETY ZONE\n(Machine stops)",
           color = "#E74C3C", fontface = "bold", size = 3.5 * diagram_text_scale) +
  # Machine
  annotate("rect", xmin = -2, xmax = 0, ymin = -2, ymax = 0,
           fill = "#95A5A6", color = "black") +
  annotate("text", x = -1, y = -1, label = "Machine", fontface = "bold", size = 4 * diagram_text_scale, color = "white") +
  # Person approaching
  annotate("point", x = 2.5, y = 0.5, size = 10, color = "#3498DB") +
  annotate("text", x = 2.5, y = 0.5, label = "O", size = 5 * diagram_text_scale, color = "white") +
  annotate("text", x = 2.5, y = -0.2, label = "Worker", size = 3 * diagram_text_scale) +
  # Scan lines
...
scanner_compare %>%
  kable(caption = "Light Curtains vs. Area Scanners") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
ggplot() +
  ...
  annotate("text", x = 6, y = 1.2, label = "Pressure", size = 3 * diagram_text_scale, color = "#E74C3C") +
  # Connection to controller
  annotate("rect", xmin = -1.5, xmax = -0.5, ymin = 0.2, ymax = 0.6,
           fill = "#2ECC71", color = "black") +
  annotate("text", x = -1, y = 0.4, label = "Safety\nRelay", size = 2.5 * diagram_text_scale) +
...
ggplot() +
  ...
  annotate("rect", xmin = 2, xmax = 6, ymin = 0, ymax = 2,
           fill = "#F1C40F", color = "black", linewidth = 1.5) +
  # Buttons
  annotate("point", x = 3, y = 1, size = 15, color = "#E74C3C") +
  annotate("point", x = 5, y = 1, size = 15, color = "#E74C3C") +
  annotate("text", x = 3, y = 1, label = "L", color = "white", fontface = "bold", size = 5 * diagram_text_scale) +
  annotate("text", x = 5, y = 1, label = "R", color = "white", fontface = "bold", size = 5 * diagram_text_scale) +
  # Hands
  annotate("point", x = 3, y = 2.5, size = 12, color = "#E67E22") +
  annotate("point", x = 5, y = 2.5, size = 12, color = "#E67E22") +
  annotate("text", x = 3, y = 2.5, label = "L", size = 4 * diagram_text_scale, color = "white") +
  annotate("text", x = 5, y = 2.5, label = "R", size = 4 * diagram_text_scale, color = "white") +
  ...
  annotate("rect", xmin = 1, xmax = 7, ymin = -2.5, ymax = -0.5,
           fill = "#95A5A6", color = "black") +
  annotate("text", x = 4, y = -1.5, label = "PRESS", fontface = "bold", size = 5 * diagram_text_scale) +
  # Time requirement
...
two_hand_req %>%
  kable(caption = "Two-Hand Control Requirements") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
  row_spec(0, bold = TRUE, background = "#E74C3C", color = "white") %>%
  column_spec(1, bold = TRUE, width = "12em")
...
ggplot() +
  ...
  annotate("rect", xmin = 0, xmax = 2.5, ymin = 0, ymax = 3,
           fill = "#FADBD8", color = "#E74C3C", linewidth = 1) +
  ...
  annotate("text", x = 1.25, y = 1.5, label = "Robot\nSTOPS",
           size = 4 * diagram_text_scale, color = "#E74C3C", fontface = "bold") +
  annotate("text", x = 1.25, y = 0.5, label = "Panic response:\nlet go = stop",
           size = 2.5 * diagram_text_scale, fontface = "italic") +
  # Position 2 - Middle
  annotate("rect", xmin = 3, xmax = 5.5, ymin = 0, ymax = 3,
           fill = "#D5F5E3", color = "#27AE60", linewidth = 1) +
  annotate("text", x = 4.25, y = 2.5, label = "Position 2\nMIDDLE",
           fontface = "bold", size = 3.5 * diagram_text_scale) +
  annotate("text", x = 4.25, y = 1.5, label = "Robot\nENABLED",
           size = 4 * diagram_text_scale, color = "#27AE60", fontface = "bold") +
  ...
  annotate("segment", x = 2.5, xend = 3, y = 1.5, yend = 1.5,
           arrow = arrow(length = unit(0.15, "cm"), ends = "both"), linewidth = 1) +
...
ggplot() +
  ...
  annotate("rect", xmin = 0.3, xmax = 3.7, ymin = 0.3, ymax = 4.7,
           fill = NA, color = "#E74C3C", linewidth = 1.5, linetype = "dashed") +
  annotate("text", x = 2, y = 4.3, label = "Safety Cage", color = "#E74C3C", size = 3 * diagram_text_scale) +
  # Robot
  annotate("point", x = 2, y = 2, size = 20, color = "#3498DB") +
  annotate("text", x = 2, y = 2, label = "Robot", color = "white", size = 3 * diagram_text_scale) +
  # Worker outside
  annotate("point", x = 0, y = 2.5, size = 10, color = "#27AE60") +
  annotate("text", x = -0.5, y = 2.5, label = "Worker\n(Outside)", size = 2.5 * diagram_text_scale) +
  ...
  annotate("text", x = 7.5, y = 2.5, label = "Cobot", color = "white", size = 3 * diagram_text_scale) +
  # Worker beside robot
  annotate("point", x = 6.3, y = 2.5, size = 10, color = "#3498DB") +
  annotate("text", x = 5.8, y = 2.5, label = "Worker\n(Beside)", size = 2.5 * diagram_text_scale) +
  ...
  annotate("text", x = 7, y = 3, label = "Force\nLimited", size = 2.5 * diagram_text_scale, color = "#F39C12") +
  theme_void() +
...
cobot_methods %>%
  kable(caption = "Four Methods of Collaborative Robot Operation") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
pfl_limits %>%
  kable(caption = "ISO/TS 15066 Biomechanical Limits (Quasi-static contact)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
  row_spec(0, bold = TRUE, background = "#9B59B6", color = "white") %>%
...
ggplot() +
  ...
  annotate("text", x = 2.9, y = -1, label = "S = Safety Distance",
           fontface = "bold", size = 4 * diagram_text_scale, color = "#3498DB") +
  # C - penetration depth
  ...
  annotate("text", x = -0.1, y = 2.5, label = "C",
           fontface = "bold", size = 4 * diagram_text_scale, color = "#9B59B6") +
  # Human approach
  annotate("point", x = -1.5, y = 2, size = 15, color = "#27AE60") +
  annotate("text", x = -1.5, y = 2, label = "O", size = 5 * diagram_text_scale, color = "white") +
  annotate("segment", x = -1, xend = 0, y = 2, yend = 2,
           arrow = arrow(length = unit(0.2, "cm")), color = "#27AE60", linewidth = 1) +
  annotate("text", x = -0.5, y = 1.5, label = "K (approach\nspeed)",
           size = 3 * diagram_text_scale, color = "#27AE60") +
  # Formula box
...
k_values %>%
  kable(caption = "Standard Approach Speed Values (ISO 13855)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
  row_spec(0, bold = TRUE, background = "#3498DB", color = "white") %>%
  column_spec(1, bold = TRUE, width = "15em")
...
c_values %>%
  kable(caption = "Penetration Depth Values Based on Resolution") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
  row_spec(0, bold = TRUE, background = "#9B59B6", color = "white")
```
...
ggplot() +
  ...
  annotate("rect", xmin = 3, xmax = 7, ymin = 1, ymax = 5,
           fill = "#F1C40F", color = "black", linewidth = 2) +
...
estop_req %>%
  kable(caption = "E-Stop Requirements per IEC 60204-1") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
  row_spec(0, bold = TRUE, background = "#E74C3C", color = "white") %>%
  column_spec(1, bold = TRUE, width = "10em")
...
ggplot() +
  ...
  annotate("rect", xmin = 0, xmax = 10, ymin = 0.8, ymax = 1.2,
           fill = "#95A5A6", color = "black") +
  annotate("text", x = 5, y = 1, label = "CONVEYOR", size = 3 * diagram_text_scale, color = "white") +
  # Pull cord
  ...
  annotate("segment", x = 5, xend = 5, y = 2, yend = 2.8,
           arrow = arrow(length = unit(0.2, "cm")), color = "#2C3E50", linewidth = 1) +
  annotate("text", x = 5, y = 3.1, label = "Pull from\nanywhere", size = 3 * diagram_text_scale) +
  # Worker positions
  annotate("point", x = c(2, 5, 8), y = rep(0.3, 3), size = 8, color = "#3498DB") +
  annotate("text", x = c(2, 5, 8), y = rep(-0.2, 3), label = "Worker", size = 2.5 * diagram_text_scale) +
  theme_void() +
...
sil_data %>%
  kable(caption = "Safety Integrity Levels (PFH = Probability of Dangerous Failure per Hour)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
pl_data %>%
  kable(caption = "Performance Levels per ISO 13849-1") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
...
ggplot(rel_data, aes(x = x, y = sil)) +
  ...
  geom_text(aes(label = pl), vjust = -0.5, fontface = "bold", size = 4 * diagram_text_scale) +
  geom_hline(yintercept = c(1, 2, 3), linetype = "dashed", color = "#7F8C8D") +
  annotate("text", x = 5.5, y = 1, label = "SIL 1", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 5.5, y = 2, label = "SIL 2", hjust = 0, size = 3 * diagram_text_scale) +
  annotate("text", x = 5.5, y = 3, label = "SIL 3", hjust = 0, size = 3 * diagram_text_scale) +
...
ggplot(steps, aes(x = 3, y = y)) +
  ...
  geom_text(aes(label = paste0("Step ", step, ": ", label)),
            color = "white", fontface = "bold", size = 4 * diagram_text_scale, lineheight = 0.8) +
  # Arrows
  ...
  annotate("text", x = 6.2, y = 3, label = "Iterate until\nrisk is\nacceptable",
           size = 3 * diagram_text_scale, color = "#E74C3C", hjust = 0, lineheight = 0.9) +
  # Side descriptions
  annotate("text", x = 0.3, y = 5, label = "Machine function,\noperator tasks,\nenvironment",
           size = 2.5 * diagram_text_scale, hjust = 1, lineheight = 0.9) +
...
ggplot(risk_matrix, aes(x = Probability, y = Severity, fill = Risk)) +
  ...
  geom_text(aes(label = Risk), fontface = "bold", size = 4 * diagram_text_scale) +
  ...
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 14 * diagram_text_scale),
    legend.position = "bottom",
    axis.text = element_text(size = 10 * diagram_text_scale, face = "bold"),
...
summary_data %>%
  kable(caption = "Machine Guarding and Safety: Key Concepts Summary") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "bordered"),
                full_width = knitr::is_html_output(), position = "center") %>%
