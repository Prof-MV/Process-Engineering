...
ggplot(cost_data, aes(x = Phase, y = Cost)) +
  ...
  geom_text(aes(label = paste0("$", Cost, "x")), vjust = -0.5, size = 4 * diagram_text_scale, fontface = "bold") +
  annotate("text", x = 3, y = 600,
           label = "Simulation catches problems HERE\nwhen changes cost $1-$10",
           color = "#27AE60", size = 4 * diagram_text_scale, fontface = "bold") +
...
ggplot() +
  ...
  geom_text(data = steps, aes(x = x, y = 0, label = label), color = "white", fontface = "bold", size = 3 * diagram_text_scale) +
  # Draw arrows
  ...
  annotate("text", x = 5, y = 0.9, label = "Entities (products) flow through the system",
           size = 4 * diagram_text_scale, fontface = "italic") +
...
ggplot() +
  ...
  geom_label(data = connections, aes(x = (x + xend)/2, y = (y + yend)/2, label = label),
             size = 2.5 * diagram_text_scale, fill = "white", label.size = 0 * diagram_text_scale) +
  ...
  geom_label(data = decisions[decisions$type == "method",],
             ...
             size = 3.5 * diagram_text_scale, fontface = "bold",
             fill = c("#E74C3C", "#2ECC71", "#F39C12", "#9B59B6")) +
  geom_text(data = decisions[decisions$type == "decision",],
            aes(x = x, y = y, label = label), size = 2.5 * diagram_text_scale, fontface = "bold") +
...
ggplot(elements, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = name), vjust = 3, size = 3 * diagram_text_scale, fontface = "bold") +
  geom_text(aes(label = description), vjust = 5, size = 2.5 * diagram_text_scale, color = "gray40") +
...
ggplot() +
  ...
  geom_text(data = sink_data, aes(x = x, y = 0.4, label = label), size = 3 * diagram_text_scale, fontface = "bold") +
  # Reject paths
  ...
  geom_text(data = reject_paths, aes(x = x, y = y - 0.3, label = label), size = 2.5 * diagram_text_scale) +
  # Legend
  annotate("rect", xmin = 13, xmax = 13.3, ymin = 0.4, ymax = 0.6, fill = "#3498DB") +
  annotate("text", x = 14, y = 0.5, label = "Source", size = 2.5 * diagram_text_scale, hjust = 0) +
  annotate("rect", xmin = 13, xmax = 13.3, ymin = 0.1, ymax = 0.3, fill = "#E74C3C") +
  annotate("text", x = 14, y = 0.2, label = "Service (Station)", size = 2.5 * diagram_text_scale, hjust = 0) +
  annotate("rect", xmin = 13, xmax = 13.3, ymin = -0.2, ymax = 0, fill = "#F39C12") +
  annotate("text", x = 14, y = -0.1, label = "Queue", size = 2.5 * diagram_text_scale, hjust = 0) +
  annotate("point", x = 13.15, y = -0.4, shape = 21, size = 4, fill = "#2ECC71") +
  annotate("text", x = 14, y = -0.4, label = "Sink", size = 2.5 * diagram_text_scale, hjust = 0) +
...
kable(strategies,
  ...
  column_spec(2, width = if (knitr::is_html_output()) "45%" else NULL) %>%
  column_spec(3, width = if (knitr::is_html_output()) "40%" else NULL)
```
...
**Key Equations:**
- Takt Time: Equation @eq-takt-time
...
Using the graph in @fig-amr-simulation as a reference:
